...
int parser_EmployeeFromText(FILE* pFile , LinkedList* pArrayListEmployee)
{
	...
	if (pFile != NULL && pArrayListEmployee != NULL)
	{
		...
		while (!feof(pFile))//leo una linea y muevo el cursor, repito hasta que se alcance el final del archivo (cuando feof no devuelva 0)
		{
			...
			ll_add(pArrayListEmployee, auxEmployee, i);//agrego el auxEmployee a la linkedlist
...
int parser_EmployeeFromBinary(FILE* pFile , LinkedList* pArrayListEmployee)
{
	...
	if (pFile != NULL && pArrayListEmployee != NULL)
	{
		while (!feof(pFile))
		{
			...
			ll_add(pArrayListEmployee, auxEmployee, i);//agrego auxEmployee a la linkedlist
			i++;
